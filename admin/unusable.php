...
if(isset($_POST['submitted']) && count($_POST['marked']) > 0) {
	...
	echo ' marked as usable: ';
	foreach ($_POST['marked'] as $key => $item) {
		$query = "UPDATE ".$mydb_tab." SET used = '0' WHERE id = '".$item."';";
    $mydb->query($query);
...
?>
  ...
  <div style="clear:both;margin:18px 0 0 0;">
	<button name="submit" type="submit" class="subbutton">Wiederherstellen</button> Alle: <input type="checkbox" onclick="selectAllCheckBoxes(this)" id="checkBoxUnik"/>
